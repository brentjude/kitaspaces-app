...
import { ArrowRightIcon, UserIcon as UserCircleIcon } from '@heroicons/react/24/outline';
...
export default function WelcomeSection({ data, upcomingEventsCount }: WelcomeSectionProps) {
    ...
    <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-gray-100">
      ...
      <div className="flex items-start justify-between mb-6 pb-6 border-b border-gray-100">
        <div className="flex items-center space-x-4">
          <div className="w-16 h-16 bg-gradient-to-br from-orange-400 to-orange-600 rounded-2xl flex items-center justify-center text-white shadow-lg">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Member ID: <span className="font-mono font-semibold text-gray-700">{user.id}</span>
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-gradient-to-br from-orange-50 to-orange-100/50 rounded-xl p-4 border border-orange-200">
          ...
          {membership ? (
            <>
              ...
              {membership.planName && (
                <p className="text-sm text-gray-600 mt-1">{membership.planName}</p>
              )}
            </>
          ) : recentPayment ? (
            <>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Last visit: {new Date(recentPayment.paidAt || '').toLocaleDateString()}
              </p>
            </>
          ) : (
        ...
        <div className="bg-gradient-to-br from-blue-50 to-blue-100/50 rounded-xl p-4 border border-blue-200">
          ...
          <p className="text-sm text-gray-600 mt-1">
            {upcomingEventsCount === 0 ? 'No events scheduled' : 'Events scheduled'}
          </p>
        </div>
      </div>
