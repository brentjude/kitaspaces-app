...
import { CheckCircleIcon, ArrowRightIcon, HomeIcon, UserPlusIcon } from '@heroicons/react/24/outline';
...
export default function ConfirmationSuccess({ confirmation, onCreateAccount }: ConfirmationSuccessProps) {
  const { data: session } = useSession();
  const { event, attendees, totalAmount, status, paymentReference } = confirmation;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        <div className="bg-white rounded-2xl shadow-sm border border-foreground/10 overflow-hidden">
          ...
          <div className="p-8 text-center space-y-6">
            ...
            <h2 className="text-2xl font-bold text-foreground">
              {isPending ? 'Registration Submitted!' : 'Registration Confirmed!'}
            </h2>
            <p className="text-foreground/60 mt-2 text-sm sm:text-base">
              {isPending ? (
                <>
                  Thank you for registering! We've received your registration and{' '}
                  {totalAmount > 0 ? (
                    <>
                      your payment is being verified. We'll send a confirmation email to{' '}
                      <span className="font-semibold text-foreground">{attendees[0].email}</span>{' '}
            ...
            <div className="max-w-md mx-auto">
              ...
              {paymentReference && (
                <div className="mb-4 p-3 bg-white rounded-lg border border-foreground/10">
                  <p className="text-xs text-foreground/60 mb-1">Payment Reference</p>
                  <p className="font-mono font-bold text-foreground text-lg">{paymentReference}</p>
              ...
              <div className="space-y-3 mb-4">
                {attendees.map((attendee, idx) => (
                  <div key={idx} className="flex justify-between text-sm p-3 bg-white rounded-lg border border-foreground/10">
                    <div>
                      <p className="font-medium text-foreground">{attendee.name}</p>
                      <p className="text-xs text-foreground/60">{attendee.email}</p>
                    </div>
                    <span className="text-foreground">
                      {totalAmount === 0 ? 'Free' : `₱${(totalAmount / attendees.length).toFixed(2)}`}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="border-t border-foreground/10 pt-3 flex justify-between items-center">
                <span className="font-bold text-foreground">Total {isPending ? 'Due' : 'Paid'}</span>
              ...
              {isPending && totalAmount > 0 && (
                <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
                  <p className="text-xs text-blue-800">
                    <span className="font-bold">Next Steps:</span> Your registration will be confirmed once payment is verified. 
                    This usually takes 1-2 business days. Please keep your payment reference <span className="font-mono font-bold">{paymentReference}</span> for tracking.
                  </p>
                </div>
              )}
            </div>
        ...
        <p className="text-center text-sm text-foreground/40 mt-6">
          ...
          <a href="mailto:support@kitaspaces.com" className="text-primary hover:underline">
