...
function EventTableRow({ event, status, isLastRow }: EventTableRowProps) {
  // 🔧 Count attendees from event_pax and customer_event_pax tables
  // Each registration = 1 primary attendee
  ...
  const customerAttendeesCount = customerPaxCount;

  // Total attendees
  const totalAttendeesCount = memberAttendeesCount + customerAttendeesCount;
        ...
        <div className="flex flex-col">
          ...
          {hasMemberDiscount && (
            <span className="flex items-center text-xs text-blue-600 mt-0.5">
              ...
              {event.memberDiscountedPrice !== null &&
              event.memberDiscountedPrice !== undefined ? (
                <>
                  ₱{event.memberDiscountedPrice.toFixed(2)} for members
                  {event.memberDiscountType === "PERCENTAGE" && (
                    <span className="ml-1 text-[10px] bg-blue-100 px-1 py-0.5 rounded">
                      {event.memberDiscount}% off
                    </span>
                  )}
                  {event.memberDiscountType === "FIXED" && (
                    <span className="ml-1 text-[10px] bg-blue-100 px-1 py-0.5 rounded">
                      ₱{event.memberDiscount} off
                    </span>
                  )}
            ...
            <div className="w-full bg-foreground/10 rounded-full h-1.5">
              <div
                className={`h-1.5 rounded-full ${
                  totalAttendeesCount >= maxAttendees
                    ? "bg-red-500"
                    : totalAttendeesCount >= maxAttendees * 0.8
                      ? "bg-yellow-500"
                      : "bg-green-500"
                }`}
                style={{
                  width: `${Math.min(
                    (totalAttendeesCount / maxAttendees) * 100,
                    100
                  )}%`,
                }}
              />
